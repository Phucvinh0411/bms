...
public interface BookSemanticRepository extends JpaRepository<fit.iuh.order.module.domain.Book, Long> {
    ...
    @Query(value = "SELECT "
        ...
        + "LEFT JOIN categories pc ON pc.id = b.parent_category_id "
    ...
    @Query(value = "SELECT "
        ...
        + "c.name as categoryName, "
        + "b.parent_category_id as parentCategoryId, "
        + "(CASE WHEN b.embedding IS NOT NULL THEN (1 - (b.embedding <=> cast(:vector as vector))) ELSE 0 END) as semanticScore, "
        + "coalesce(ts_rank_cd(b.fts_tokens, plainto_tsquery('simple', :query)), 0) as ftsScore, "
        + "(CASE WHEN b.title ILIKE concat('%', :query, '%') THEN 1 ELSE 0 END) as titleScore, "
        + "(CASE WHEN b.author ILIKE concat('%', :query, '%') THEN 1 ELSE 0 END) as authorScore, "
        + "(CASE WHEN b.publisher ILIKE concat('%', :query, '%') THEN 1 ELSE 0 END) as publisherScore, "
        + "(CASE WHEN b.description ILIKE concat('%', :query, '%') THEN 1 ELSE 0 END) as descriptionScore, "
        + "(CASE WHEN c.name ILIKE concat('%', :query, '%') OR pc.name ILIKE concat('%', :query, '%') THEN 1 ELSE 0 END) as categoryScore, "
        + "((CASE WHEN b.embedding IS NOT NULL THEN (1 - (b.embedding <=> cast(:vector as vector))) ELSE 0 END) * 0.3 + "
        + "(coalesce(ts_rank_cd(b.fts_tokens, plainto_tsquery('simple', :query)), 0) * 0.35) + "
        + "(CASE WHEN b.title ILIKE concat('%', :query, '%') THEN 0.12 ELSE 0 END) + "
        + "(CASE WHEN b.author ILIKE concat('%', :query, '%') THEN 0.08 ELSE 0 END) + "
        + "(CASE WHEN b.publisher ILIKE concat('%', :query, '%') THEN 0.05 ELSE 0 END) + "
        + "(CASE WHEN b.description ILIKE concat('%', :query, '%') THEN 0.05 ELSE 0 END) + "
        + "(CASE WHEN c.name ILIKE concat('%', :query, '%') OR pc.name ILIKE concat('%', :query, '%') THEN 0.05 ELSE 0 END)) as combinedScore "
        + "FROM books b "
        + "LEFT JOIN categories c ON c.id = b.category_id "
        + "LEFT JOIN categories pc ON pc.id = b.parent_category_id "
        ...
        + "AND (b.embedding IS NOT NULL OR b.fts_tokens IS NOT NULL OR b.title ILIKE concat('%', :query, '%') "
    ...
    @Query(value = "SELECT "
        ...
        + "      WHEN c.name ILIKE concat('%', :query, '%') THEN 0.7 "
        + "      WHEN b.description ILIKE concat('%', :query, '%') THEN 0.5 "
