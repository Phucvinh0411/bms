package fit.iuh.modules.order.service;

import fit.iuh.modules.checkout.model.CheckoutContext;
import fit.iuh.modules.checkout.handler.AddressSelectionHandler;
import fit.iuh.modules.checkout.handler.PersistOrderHandler;
import fit.iuh.modules.checkout.handler.PricingHandler;
import fit.iuh.modules.checkout.handler.StockCheckHandler;
import fit.iuh.modules.checkout.handler.VoucherCheckHandler;
import fit.iuh.modules.order.dto.*;
...
import fit.iuh.modules.payment.strategy.PayOSPaymentStrategy;
import fit.iuh.modules.payment.model.PaymentTransaction;
import fit.iuh.modules.order.state.*;
...
public class OrderService {
    ...
    private final fit.iuh.modules.payment.repository.PaymentTransactionRepository paymentTransactionRepository;
    private final fit.iuh.infrastructure.messaging.OrderEventPublisher orderEventPublisher;
    private final fit.iuh.infrastructure.messaging.OrderMessageProducer orderMessageProducer;
    ...
    public OrderService(
        ...
        @org.springframework.beans.factory.annotation.Autowired(required = false) PayOSPaymentStrategy payOSPaymentStrategy,
        @org.springframework.beans.factory.annotation.Autowired(required = false) fit.iuh.modules.payment.repository.PaymentTransactionRepository paymentTransactionRepository,
        @org.springframework.beans.factory.annotation.Autowired(required = false) fit.iuh.infrastructure.messaging.OrderEventPublisher orderEventPublisher,
        @org.springframework.beans.factory.annotation.Autowired(required = false) fit.iuh.infrastructure.messaging.OrderMessageProducer orderMessageProducer
    ) {
    ...
    public OrderResponse createOrder(OrderRequest request) {
        ...
        if ("PAYOS".equalsIgnoreCase(request.getPaymentMethod()) && payOSPaymentStrategy != null) {
            ...
            if (paymentTransactionRepository != null) {
                ...
                transaction.setStatus(fit.iuh.domain.model.PaymentStatus.UNPAID);
                transaction.setIsDeleted(false);
                paymentTransactionRepository.save(transaction);
            }

            if (orderMessageProducer != null) {
                orderMessageProducer.enqueueOrderTimeoutCheck(savedOrder.getId());
            }
    ...
    public OrderResponse cancelOrder(Long id) {
        ...
        // Xác định State hiện tại bằng State Pattern động dựa trên trạng thái của Order
        OrderState currentState = switch (order.getStatus()) {
            case PENDING, AWAITING_PAYMENT -> new PendingState();
            case CONFIRMED, SHIPPING -> new ShippingState();
    ...
    public OrderResponse changePaymentMethod(Long id, String paymentMethod, String returnUrl, String cancelUrl) {
        ...
        if ("PAYOS".equalsIgnoreCase(paymentMethod)) {
            ...
            if (paymentTransactionRepository != null) {
                ...
                if (txOpt.isPresent()) {
                    var tx = txOpt.get();
                    if (tx.getStatus() == fit.iuh.domain.model.PaymentStatus.PAID) {
            ...
            if (orderMessageProducer != null) {
                orderMessageProducer.enqueueOrderTimeoutCheck(order.getId());
            }
